...
class Model:
    def __init__(
        ...
        """
        if not model_config_log_path is None:
            os.makedirs(model_config_log_path, exist_ok=True)
            file_handle = open(
                model_config_log_path + "/Parameters_Record.txt", mode="a"
            )
    ...
    def train(
        ...
        writer_train = SummaryWriter(f"{self.tensorboard_path}/train")
        writer_val = SummaryWriter(f"{self.tensorboard_path}/val")
        ...
        self.train_dataset = RNA_ATAC_dataset(
            self.RNA_data, self.ATAC_data, train_id_r, train_id_a
        )
        self.validation_dataset = RNA_ATAC_dataset(
            self.RNA_data, self.ATAC_data, validation_id_r, validation_id_a
        )
        ...
        with tqdm(total=R2R_pretrain_epoch, ncols=100) as pbar:
            ...
            for epoch in range(R2R_pretrain_epoch):
                ...
                writer_train.add_scalar(
                    "pretrain/control/loss/r_loss", pretrain_r_loss_mean, epoch
                )
                writer_train.add_scalar(
                    "pretrain/control/loss/kl", pretrain_r_kl_mean, epoch
                )
                writer_val.add_scalar(
                    "pretrain/control/loss/r_loss", pretrain_r_loss_val_mean, epoch
                )
                writer_val.add_scalar(
                    "pretrain/control/loss/kl", pretrain_r_kl_val_mean, epoch
                )
                ...
                if self.early_stopping_R2R.early_stop:
                    my_logger.info(
                        "Control pretraining early stop, validation loss does not improve in "
                        + str(patience)
                        + " epoches!"
                    )
                    self.RNA_encoder.load_state_dict(
        ...
        with tqdm(total=A2A_pretrain_epoch, ncols=100) as pbar:
            ...
            for epoch in range(A2A_pretrain_epoch):
                ...
                writer_train.add_scalar(
                    "pretrain/stimulated/loss/r_loss", pretrain_r_loss_mean, epoch
                )
                writer_train.add_scalar(
                    "pretrain/stimulated/loss/kl", pretrain_r_kl_mean, epoch
                )
                writer_val.add_scalar(
                    "pretrain/stimulated/loss/r_loss", pretrain_r_loss_val_mean, epoch
                )
                writer_val.add_scalar(
                    "pretrain/stimulated/loss/kl", pretrain_r_kl_val_mean, epoch
                )
                ...
                if self.early_stopping_A2A.early_stop:
                    my_logger.info(
                        "Stimulated pretraining early stop, validation loss does not improve in "
                        + str(patience)
                        + " epoches!"
                    )
                    self.ATAC_encoder.load_state_dict(
        ...
        with tqdm(total=translator_epoch, ncols=100) as pbar:
            ...
            for epoch in range(translator_epoch):
                ...
                writer_train.add_scalar(
                    "integrative/loss/r_loss", train_loss_mean, epoch
                )
                writer_train.add_scalar("integrative/loss/kl", train_kl_mean, epoch)
                writer_train.add_scalar(
                    "integrative/loss/discriminator", train_discriminator_mean, epoch
                )
                writer_train.add_scalar(
                    "integrative/loss/generator", train_generator_mean, epoch
                )
                writer_val.add_scalar(
                    "integrative/loss/r_loss", train_loss_val_mean, epoch
                )
                writer_val.add_scalar("integrative/loss/kl", train_kl_val_mean, epoch)
                writer_val.add_scalar(
                    "integrative/loss/discriminator",
                    train_discriminator_val_mean,
                    epoch,
                )
                writer_val.add_scalar(
                    "integrative/loss/generator", train_generator_val_mean, epoch
                )
    ...
    def test(
        ...
        """
        my_logger = create_logger(
            name="Tester", ch=True, fh=False, levelname=logging.INFO, overwrite=False
        )
        ...
        self.R_test_dataloader = DataLoader(
            self.R_test_dataset,
            batch_size=100,
            shuffle=False,
            num_workers=self.num_workers,
        )
        self.A_test_dataloader = DataLoader(
            self.A_test_dataset,
            batch_size=100,
            shuffle=False,
            num_workers=self.num_workers,
