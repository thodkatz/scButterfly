...
def draw_reg_plot(
    ...
    if top_gene_list is not None:
        ...
        r_top_100 = round(
            data_deg_100["case"].corr(data_deg_100["predict"], method="pearson"), 3
        )
        r_top_20 = round(
            data_deg_20["case"].corr(data_deg_20["predict"], method="pearson"), 3
        )
        ...
        ax.text(
            xt,
            yt,
            s="$R^2_{top 100 genes}$=" + str(round(r_top_100 * r_top_100, 3)),
            fontsize=fontsize,
            color="black",
        )
    ...
    if title:
        ...
        ax.set_title(
            "The Linear Regression of True and Predict Expression "
            + reg_type
            + " of "
            + cell_type
        )
    ...
    return (
        round(r * r, 3),
        round(r_top_100 * r_top_100, 3),
        round(r_top_20 * r_top_20, 3),
        fig,
    )
